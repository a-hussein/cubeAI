...
def test_iterate_through_scrambles_for_testing():
    ...
    assert (
        do_scramble(
            ["L2"], iterate_through_scrambles_for_testing(scrambles)[0]
        ).cube_state
        == iterate_through_scrambles_for_testing(scrambles)[1].cube_state
    )
